...
export const InventoryView:React.FC<{
...
}>=({items,movements,onSave,onDelete,onMove,onDeleteMovement})=>{
 ...
 const [skuToDelete,setSkuToDelete]=useState<InventoryItem|null>(null);
 ...
 return <div className="space-y-5 max-w-7xl mx-auto">
  ...
  <section className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden">
   <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row gap-3 sm:items-center justify-between"><div className="flex items-center gap-2"><History className="w-5 h-5 text-amber-500"/><div><h2 className="font-black">Histórico de movimentações</h2><p className="text-xs text-slate-500">Livro razão do estoque: o que entrou, saiu e foi transferido.</p></div></div><select value={historySku} onChange={e=>setHistorySku(e.target.value)} className="rounded-xl border border-slate-300 dark:border-slate-700 bg-transparent px-3 py-2 text-sm"><option className="bg-white text-slate-900 dark:bg-slate-900 dark:text-white" value="ALL">Todos os SKUs</option>{items.map(i=><option className="bg-white text-slate-900 dark:bg-slate-900 dark:text-white" key={i.id} value={i.id}>{i.sku} — {i.name}</option>)}</select></div>
   <div className="divide-y divide-slate-100 dark:divide-slate-800">{filtered.length===0?<div className="p-8 text-center text-sm text-slate-500">Nenhuma movimentação registrada ainda.</div>:filtered.slice(0,100).map(m=><div key={m.id} className="p-4 grid sm:grid-cols-[1fr_auto] gap-2"><div><div className="flex flex-wrap items-center gap-2"><span className="font-black text-xs text-amber-600">{m.sku}</span><span className="font-bold text-sm">{labels[m.type]}</span><span className="text-xs rounded-full bg-slate-100 dark:bg-slate-800 px-2 py-0.5">{m.quantity} un.</span></div><p className="text-xs text-slate-500 mt-1">{m.productName}{m.note?` • ${m.note}`:''}</p></div><div className="flex items-center sm:justify-end gap-2"><span className="text-xs text-slate-500">{new Date(m.createdAt).toLocaleString('pt-BR')}</span><button title="Excluir movimentação e reverter estoque" onClick={()=>setMovementToDelete(m)} className="p-2 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/30 transition-colors"><Trash2 className="w-4 h-4"/></button></div></div>)}</div>
  </section>
  ...
  {moving&&<div className="fixed inset-0 z-[80] bg-slate-950/75 p-3 grid place-items-center"><div className="w-full max-w-xl rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 overflow-hidden"><div className="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between"><div><h2 className="font-black">Movimentar {moving.sku}</h2><p className="text-xs text-slate-500">{moving.name}</p></div><button onClick={()=>setMoving(null)}><X className="w-5 h-5"/></button></div><div className="p-4 space-y-4">
   <label className="text-xs font-bold block">Tipo de movimentação<select value={moveType} onChange={e=>setMoveType(e.target.value as InventoryMovementType)} className="mt-1 w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-transparent px-3 py-3 text-sm">{(Object.keys(labels) as InventoryMovementType[]).map(k=><option className="bg-white text-slate-900 dark:bg-slate-900 dark:text-white" key={k} value={k}>{labels[k]} — {desc[k]}</option>)}</select></label>
  ...
  {skuToDelete&&<div className="fixed inset-0 z-[100] bg-slate-950/60 backdrop-blur-sm p-4 flex items-center justify-center" onClick={()=>!deleting&&setSkuToDelete(null)}>
   <div className="w-full max-w-md rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl p-5 space-y-4" onClick={e=>e.stopPropagation()}>
    <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 flex items-center justify-center border border-rose-200 dark:border-rose-800"><Trash2 className="w-5 h-5"/></div><div><h3 className="text-base font-black">Excluir SKU</h3><p className="text-xs text-slate-600 dark:text-slate-400 font-medium">Deseja realmente excluir este SKU do estoque?</p></div></div>
    <div className="p-3 bg-slate-50 dark:bg-slate-800/70 rounded-xl border border-slate-200 dark:border-slate-700"><strong className="text-xs font-bold block">{skuToDelete.sku} — {skuToDelete.name}</strong><span className="text-3xs text-slate-500 block mt-1">Local: {skuToDelete.localStock} • Full: {skuToDelete.fullStock} • A receber: {skuToDelete.supplierInbound} • → Full: {skuToDelete.fullInbound}</span></div>
    <div className="flex justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-800"><button disabled={deleting} onClick={()=>setSkuToDelete(null)} className="px-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 font-bold text-xs">Cancelar</button><button disabled={deleting} onClick={async()=>{setDeleting(true);try{await onDelete(skuToDelete.id);setSkuToDelete(null)}finally{setDeleting(false)}}} className="px-4 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-black text-xs disabled:opacity-50">{deleting?'Excluindo...':'Excluir'}</button></div>
   </div>
  </div>}

  {movementToDelete&&<div className="fixed inset-0 z-[100] bg-slate-950/60 backdrop-blur-sm p-4 flex items-center justify-center" onClick={()=>!deleting&&setMovementToDelete(null)}>
   <div className="w-full max-w-md rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl p-5 space-y-4" onClick={e=>e.stopPropagation()}>
    <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 flex items-center justify-center border border-rose-200 dark:border-rose-800"><Trash2 className="w-5 h-5"/></div><div><h3 className="text-base font-black">Excluir Movimentação</h3><p className="text-xs text-slate-600 dark:text-slate-400 font-medium">O efeito desta movimentação será desfeito no estoque.</p></div></div>
    <div className="p-3 bg-slate-50 dark:bg-slate-800/70 rounded-xl border border-slate-200 dark:border-slate-700"><strong className="text-xs font-bold block">{movementToDelete.sku} — {movementToDelete.productName}</strong><span className="text-3xs text-slate-500 block mt-1">{labels[movementToDelete.type]} • {movementToDelete.quantity} un.</span></div>
    <div className="flex justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-800"><button disabled={deleting} onClick={()=>setMovementToDelete(null)} className="px-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 font-bold text-xs">Cancelar</button><button disabled={deleting} onClick={async()=>{setDeleting(true);try{await onDeleteMovement(movementToDelete.id);setMovementToDelete(null)}finally{setDeleting(false)}}} className="px-4 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-black text-xs disabled:opacity-50">{deleting?'Excluindo...':'Excluir e reverter'}</button></div>
   </div>
  </div>}
 </div>;
};
