...
export const FinanceCenterView: React.FC<FinanceCenterViewProps> = ({ onLogout }) => {
  ...
  const adicionarContaFixaObrigatoria = async () => {
    ...
    const novasContas = [...contasFixasObrigatorias, { id: Date.now().toString(), nome: novaContaNome, valor: Number(novaContaValor), mesesTotales: meses, mesesRestantes: meses }];
    setContasFixasObrigatorias(novasContas);
    setNovaContaNome(''); setNovaContaValor(''); setNovaContaMeses('');
  ...
  const solicitarAdicaoPote = (pote: Pote) => {
    if (!potesAtivos.some(p => p.id === pote.id)) {
      if (disponivelGeral <= 0) { alert("100% do saldo restante já foi mapeado! Reduza a porcentagem de outro pote primeiro."); return; }
  ...
  const confirmarAdicionarPote = async () => {
    if (potePendente) {
      ...
      if (novoConjunto.reduce((acc, p) => acc + p.percentual, 0) > 100) { alert("A soma dos potes não pode ultrapassar 100%!"); return; }
  ...
  const salvarLancamento = async () => {
    ...
    if (tipoLancamento === 'entrada') {
      ...
      const novaSaida: Transacao = { id: Date.now().toString(), descricao: categoriaSaidaSelecionada, valor: valorGasto, tipo: 'saida', poteId: poteSelecionadoId, poteNome: nomePoteFinal, data: new Date().toLocaleDateString('pt-BR') };
      const novasTransacoes = [novaSaida, ...transacoes];
  ...
  const efetivarAporteSobraPatrimonio = async () => {
    ...
    if (valorTransf > saldoUnicoReal) {
      alert("O valor escolhido é maior que o seu Saldo Real Disponível atual!");
      return;
    }

    const poteIdDestino = destinoAporteSobra === 'nosso_patrimonio' ? 'aporte_extra_nosso_patrimonio' : 'aporte_extra_patrimonio_manuela';
    const poteNomeDestino = destinoAporteSobra === 'nosso_patrimonio' ? 'Nosso Patrimônio (Aporte Extra)' : 'Poupança Manuela (Aporte Extra)';
    ...
    const novasTransacoes = [novaSaida, ...transacoes];
    setTransacoes(novasTransacoes);
    setValorAporteSobra(''); setModalAporteSobra(false);
    await salvarDadosNaNuvem({ transacoes: novasTransacoes });
  };
  ...
  const adicionarMeta = async () => {
    ...
    if (valorGuardadoInicial > 0) {
      if (valorGuardadoInicial > saldoUnicoReal) { alert("O valor inicial guardado não pode ser maior que o Saldo Real disponível!"); return; }
      const novaSaida: Transacao = { id: Date.now().toString(), descricao: `Aporte inicial Meta: ${nomeNovaMeta}`, valor: valorGuardadoInicial, tipo: 'saida', poteId: 'meta_aporte', poteNome: `Meta: ${nomeNovaMeta}`, data: new Date().toLocaleDateString('pt-BR') };
      novasTransacoes = [novaSaida, ...novasTransacoes];
      setTransacoes(novasTransacoes);
    }

    const novasMetas = [...metas, { id: Date.now().toString(), nome: nomeNovaMeta, valorAlvo: Number(valorNovaMeta), valorGuardado: valorGuardadoInicial, meses: Number(mesesNovaMeta) }];
  ...
  const efetivarTransferenciaMeta = async () => {
    ...
    if (origemDepositoMeta === 'disponivel') {
      ...
      const novaSaida: Transacao = { id: Date.now().toString(), descricao: `Transferência para Meta`, valor: valorTransf, tipo: 'saida', poteId: 'meta_transf', poteNome: 'Depósito em Meta', data: new Date().toLocaleDateString('pt-BR') };
      novasTransacoes = [novaSaida, ...novasTransacoes];
    } else {
      ...
      const saidaPatri: Transacao = { id: Date.now().toString(), descricao: `Resgate de ${nomeOrigemPatri} para Meta`, valor: valorTransf, tipo: 'saida', poteId: origemDepositoMeta, poteNome: nomeOrigemPatri, data: new Date().toLocaleDateString('pt-BR') };
  ...
  if (carregandoNuvem) return <div className={`min-h-screen ${bgClasse} flex items-center justify-center font-bold text-sm`}>Carregando...</div>;
  ...
  return (
    <div className={`min-h-screen ${bgClasse} font-sans tracking-tight flex flex-col justify-between transition-colors duration-300 pb-28 select-none`}>
        ...
        <main className="max-w-4xl mx-auto p-3 md:p-6 w-full space-y-4 md:space-y-6">
          ...
          <div className={`${cardClasse} rounded-3xl p-4 md:p-6 space-y-4 border-2 border-rose-500/20`}>
            ...
            <div className="space-y-2">
              <input type="text" placeholder="Nome da Dívida / Conta Fixa (Ex: Aluguel, Empréstimo)" value={novaContaNome} onChange={(e) => setNovaContaNome(e.target.value)} className={`w-full ${inputBg} p-3 rounded-2xl text-xs font-bold border`} />
              <div className="grid grid-cols-2 gap-2">
                <input type="number" placeholder="Valor Mensal R$" value={novaContaValor} onChange={(e) => setNovaContaValor(e.target.value === '' ? '' : Number(e.target.value))} className={`w-full ${inputBg} p-3 rounded-2xl text-xs font-bold border font-mono`} />
              ...
              <button type="button" onClick={adicionarContaFixaObrigatoria} className="w-full bg-rose-500 hover:bg-rose-600 text-white font-bold py-3 rounded-2xl text-xs flex items-center justify-center gap-1.5 cursor-pointer">
              ...
              <div className="space-y-2 pt-2 max-h-48 overflow-y-auto">
                {contasFixasObrigatorias.map(c => (
                  <div key={c.id} className={`${inputBg} p-3 rounded-2xl flex justify-between items-center text-xs border`}>
                    ...
                    <button onClick={() => removerContaFixaObrigatoria(c.id)} className="text-rose-500 cursor-pointer"><Trash2 className="w-4 h-4" /></button>
                  </div>
                ))}
              </div>
            )}
              ...
              <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
                ...
                {(() => {
                  ...
                  return potesAtivos.map(pote => {
                    ...
                    return <circle key={pote.id} cx="50" cy="50" r="40" fill="transparent" stroke={pote.cor} strokeWidth="11" strokeDasharray={dashArray} strokeDashoffset={dashOffset} className="transition-all duration-300 ease-out" />;
                  });
                })()}
              </svg>
            ...
            <div className="w-full space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-2.5 w-full">
                {potesAtivos.map(pote => (
                  <div key={pote.id} onClick={() => { setPotePendente(pote); setPercentualPendente(pote.percentual); }} className={`${cardClasse} p-3.5 rounded-2xl flex flex-col items-center text-center cursor-pointer relative group transition-all active:scale-95 hover:border-emerald-500/50`}>
                    <button onClick={(e) => { e.stopPropagation(); removerPote(pote.id); }} className="absolute top-1.5 right-1.5 text-slate-400 hover:text-rose-500 cursor-pointer"><X className="w-3.5 h-3.5" /></button>
              ...
              {totalMapeado === 100 ? (
                <button onClick={() => navegarPara('dashboard')} className="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-black py-4 rounded-2xl shadow-lg shadow-emerald-500/20 transition-all text-sm md:text-base cursor-pointer">Salvar Plano e Ir para o Painel</button>
              ) : (
          ...
          <div className="space-y-3 pt-2">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2.5">
              {todosPotesDisponiveis.map(pote => {
                ...
                return (
                  <button key={pote.id} disabled={selecionado} onClick={() => solicitarAdicaoPote(pote)} className={`p-3 rounded-2xl border text-center flex flex-col items-center space-y-1.5 transition-all ${selecionado ? 'opacity-40 border-slate-200 grayscale cursor-not-allowed bg-slate-900/40' : `${cardClasse} hover:border-emerald-500 active:scale-95 cursor-pointer`}`}>
        ...
        return (
          <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className={`${cardClasse} rounded-3xl p-5 md:p-6 max-w-sm w-full text-center space-y-4 shadow-2xl relative`}>
              <button onClick={() => setPotePendente(null)} className="absolute top-4 right-4 text-slate-400 cursor-pointer"><X className="w-5 h-5" /></button>
              ...
              <div><h3 className="text-base md:text-lg font-black">{potePendente.nome}</h3><span className="text-xs text-slate-400">Porcentagem do saldo restante (Máx: {maxPermitido}%)</span></div>
              ...
              <input type="range" min="0" max={maxPermitido} value={percentualPendente} onChange={(e) => setPercentualPendente(Number(e.target.value))} className="w-full h-2 bg-slate-200 dark:bg-slate-800 rounded-lg appearance-none cursor-pointer accent-emerald-500" />
              <button onClick={confirmarAdicionarPote} className="w-full bg-emerald-500 text-white font-black py-3 rounded-2xl shadow-lg shadow-emerald-500/20 flex items-center justify-center gap-2 text-sm cursor-pointer"><Check className="w-4 h-4" /> Confirmar Porcentagem</button>
            </div>
          </div>
        );
      })()}
      ...
      {animacaoEntrada && animacaoEntrada.ativo && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className={`${cardClasse} rounded-3xl p-6 max-w-md w-full text-center space-y-5 shadow-2xl relative border-2 border-emerald-500/40 animate-fade-in max-h-[90vh] overflow-y-auto`}>
            <div className="w-16 h-16 mx-auto rounded-2xl bg-emerald-500/20 text-emerald-500 flex items-center justify-center animate-bounce"><Sparkles className="w-8 h-8" /></div>
            <div>
            ...
            <button onClick={() => setAnimacaoEntrada(null)} className="w-full bg-emerald-500 text-white font-black py-3.5 rounded-2xl shadow-lg shadow-emerald-500/20 text-sm cursor-pointer">Ver Meu Painel</button>
          </div>
        </div>
      )}
        ...
        <main className="max-w-5xl mx-auto p-3 md:p-6 w-full space-y-4 md:space-y-6">
          ...
          {aportePendenteValor !== '' && Number(aportePendenteValor) > 0 && (
            <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-3xl p-4 flex flex-col sm:flex-row items-center justify-between gap-3 text-emerald-400">
              ...
              <button onClick={() => setModalAportePendente(true)} className="bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-black px-4 py-2 rounded-2xl text-xs transition-all shrink-0 shadow-md cursor-pointer">Ajustar Valor</button>
            </div>
          )}

          {/* ÚNICO SALDO REAL DA CONTA */}
          <div className={`${cardClasse} rounded-3xl p-6 md:p-8 flex flex-col justify-between space-y-4 border-2 border-emerald-500/40 relative overflow-hidden`}>
            <div>
            ...
            <span className={`text-[10px] md:text-xs font-semibold ${textMuted}`}>
              Valores retidos, gastos manuais e aportes extras já subtraídos. O que está aqui é dinheiro livre.
            </span>
            <div className="flex flex-col sm:flex-row gap-3 mt-2">
              <button onClick={() => setModalAporteSobra(true)} className="bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 font-bold px-4 py-2.5 rounded-2xl text-xs flex items-center justify-center gap-2 transition-all cursor-pointer shadow-sm">
                <Vault className="w-4 h-4" /> Aportar Sobra no Patrimônio
              </button>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-3 pt-2">
            <div className="flex items-center gap-2">
              ...
              <div className={`flex ${inputBg} p-1 rounded-xl border`}>
                <button onClick={() => setModoVisualizacaoPotes('coluna')} className={`p-1.5 rounded-lg cursor-pointer ${modoVisualizacaoPotes === 'coluna' ? 'bg-emerald-500 text-white' : textMuted}`}><Columns2 className="w-4 h-4" /></button>
                <button onClick={() => setModoVisualizacaoPotes('grid')} className={`p-1.5 rounded-lg cursor-pointer ${modoVisualizacaoPotes === 'grid' ? 'bg-emerald-500 text-white' : textMuted}`}><LayoutGrid className="w-4 h-4" /></button>
              </div>
            </div>
                ...
                <div key={pote.id} className={`${cardClasse} rounded-3xl p-5 md:p-6 flex flex-col items-center text-center space-y-4 relative overflow-hidden`}>
                  ...
                  <div className="relative w-32 h-32 md:w-36 md:h-36 flex items-center justify-center drop-shadow-md">
                    <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90 absolute inset-0">
                      ...
                      <circle cx="50" cy="50" r="40" fill="transparent" stroke={saldoRealPote < 0 ? '#EF4444' : pote.cor} strokeWidth="10" strokeDasharray="251.327" strokeDashoffset={dashOffset} strokeLinecap="round" className="transition-all duration-700 ease-out" />
                    </svg>
                    <div className="flex flex-col items-center justify-center z-10">
                      <span className={`text-sm md:text-base font-black font-mono ${saldoRealPote < 0 ? 'text-rose-500' : ''}`}>{formatarGrana(saldoRealPote)}</span>
                      <span className="text-[9px] text-slate-400">Limite disponível</span>
                    </div>
                  </div>
                  <div className="space-y-0.5 w-full">
                    <span className={`text-xs font-bold block ${textMuted}`}>Alocado ({pote.percentual}% do total): {formatarGrana(valorDiluidoNoPote)}</span>
                  </div>
                </div>
              );
            })}
        ...
        <main className="max-w-2xl mx-auto p-3 md:p-6 w-full space-y-4 md:space-y-6">
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <button onClick={() => setModalAporteSobra(true)} className="bg-emerald-500/10 text-emerald-500 px-3.5 py-2 rounded-2xl text-xs font-bold cursor-pointer hidden md:block border border-emerald-500/30">
                + Aportar Sobra
              </button>
          ...
          <button onClick={() => setModalAporteSobra(true)} className="w-full md:hidden bg-emerald-500/10 text-emerald-500 px-3.5 py-3 rounded-2xl text-xs font-bold cursor-pointer border border-emerald-500/30 flex items-center justify-center gap-2">
            <Vault className="w-4 h-4" /> + Aportar Sobra do Saldo Aqui
          </button>
          ...
          <div className={`${cardClasse} rounded-3xl p-5 md:p-6 text-center space-y-2 border-2 border-emerald-500/30`}>
            ...
            <span className={`text-[10px] ${textMuted}`}>Soma dos potes de patrimônio automático + aportes extras + bens manuais</span>
          </div>
          ...
          <div className="space-y-3">
            <div className={`${cardClasse} rounded-2xl p-4 flex justify-between items-center border-l-4 border-l-emerald-500`}>
              <div className="flex items-center space-x-3">
                <span className="text-2xl">🐷</span>
                <div>
                  <span className="font-bold block text-sm">Nosso Patrimônio (Pote)</span>
                  <span className={`text-[10px] ${textMuted}`}>Alimentado automaticamente + Aportes Extras</span>
                </div>
              </div>
            ...
            <div className={`${cardClasse} rounded-2xl p-4 flex justify-between items-center border-l-4 border-l-cyan-500`}>
              <div className="flex items-center space-x-3">
                <span className="text-2xl">👶</span>
                <div>
                  <span className="font-bold block text-sm">Poupança Manuela</span>
            ...
            {itensPatrimonioManuais.map(item => (
              <div key={item.id} className={`${cardClasse} rounded-2xl p-4 flex justify-between items-center border-l-4 border-l-amber-500`}>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <button onClick={() => removerPatrimonioManual(item.id)} className="text-slate-400 hover:text-rose-500 cursor-pointer"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
            ))}
          </div>
      ...
      {telaAtiva === 'dividas' && (
        <main className="max-w-2xl mx-auto p-3 md:p-6 w-full space-y-4 md:space-y-6">
          ...
          <div className={`${cardClasse} rounded-3xl p-5 md:p-6 text-center space-y-2 border-2 border-rose-500/30`}>
            ...
            <div className="text-3xl md:text-4xl font-black text-rose-500 font-mono">{formatarGrana(totalContasFixasValor)}</div>
            <span className={`text-[10px] ${textMuted}`}>({percentualComprometidoDividas.toFixed(1)}% da renda mensal)</span>
          </div>
          <div className="space-y-3">
            {contasFixasObrigatorias.length === 0 ? (
              <p className={`text-center text-xs py-10 ${textMuted}`}>Nenhuma dívida cadastrada.</p>
            ) : (
              contasFixasObrigatorias.map(c => {
                ...
                return (
                  <div key={c.id} className={`${cardClasse} rounded-3xl p-5 space-y-3 border border-rose-500/20`}>
                    <div className="flex justify-between items-center">
                      ...
                      <button onClick={() => pagarParcelaDivida(c.id)} disabled={c.mesesRestantes <= 0} className={`px-3.5 py-2 rounded-xl text-xs font-black flex items-center gap-1.5 cursor-pointer shadow-md ${c.mesesRestantes <= 0 ? 'bg-slate-800 text-slate-500 cursor-not-allowed' : 'bg-rose-500 hover:bg-rose-600 text-white'}`}><Check className="w-4 h-4" /> Pagar</button>
                    </div>
                    <div className="space-y-1">
                      <div className="flex justify-between text-xs font-bold"><span className={textMuted}>Progresso ({progressoMeses.toFixed(0)}% quitado)</span><span className="font-mono text-rose-400">{c.mesesRestantes} de {c.mesesTotales} restam</span></div>
                      <div className="w-full h-2.5 bg-slate-800 rounded-full overflow-hidden"><div className="h-full bg-rose-500 transition-all duration-500" style={{ width: `${progressoMeses}%` }}></div></div>
                    </div>
                    <div className="flex justify-between items-center pt-2 border-t border-slate-800 text-xs"><span className={textMuted}>Valor Total Restante da Dívida:</span><span className="font-mono font-black text-rose-400 text-sm">{formatarGrana(c.valor * c.mesesRestantes)}</span></div>
                  </div>
                );
              })
            )}
      ...
      {telaAtiva === 'metas' && (
        <main className="max-w-2xl mx-auto p-3 md:p-6 w-full space-y-4 md:space-y-6">
          <div className="flex justify-between items-center">
            ...
            <button onClick={() => setModalNovaMeta(true)} className="bg-emerald-500 text-white px-3.5 py-2 rounded-2xl text-xs font-bold cursor-pointer">+ Nova Meta</button>
          </div>
          <div className="space-y-3">
            {metas.length === 0 ? (
              <p className={`text-center text-xs py-10 ${textMuted}`}>Nenhuma meta cadastrada.</p>
            ) : (
              metas.map(meta => {
                ...
                return (
                  <div key={meta.id} className={`${cardClasse} rounded-3xl p-5 space-y-3 border border-emerald-500/20`}>
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center space-x-2">
                        <button onClick={() => { setMetaSelecionadaId(meta.id); setModalDepositoMeta(true); }} className="bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1 cursor-pointer"><ArrowUpRight className="w-3.5 h-3.5" /> + Depositar</button>
                        <button onClick={() => removerMeta(meta.id)} className="text-slate-400 hover:text-rose-500 cursor-pointer"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </div>
                    <div className="space-y-1">
                      <div className="flex justify-between text-xs font-bold"><span className={textMuted}>Progresso ({progressoPct.toFixed(1)}%)</span><span className="font-mono text-emerald-500">{formatarGrana(meta.valorGuardado)} / {formatarGrana(meta.valorAlvo)}</span></div>
                      <div className="w-full h-2.5 bg-slate-800 rounded-full overflow-hidden"><div className="h-full bg-emerald-500 transition-all duration-500" style={{ width: `${progressoPct}%` }}></div></div>
                    </div>
                    <div className="grid grid-cols-2 gap-3 pt-2 border-t border-slate-800">
                      <div><span className={`text-[10px] uppercase font-bold block ${textMuted}`}>Falta Concluir</span><span className="font-mono font-black text-rose-400 text-sm">{formatarGrana(valorFaltante)}</span></div>
                      <div><span className={`text-[10px] uppercase font-bold block ${textMuted}`}>Prazo Restante</span><span className="font-mono font-black text-sm">{meta.meses} meses</span></div>
                    </div>
                  </div>
                );
              })
        ...
        <main className="max-w-xl mx-auto p-3 md:p-6 w-full space-y-4">
          ...
          <div className="space-y-2">
            {transacoesFiltradas.length === 0 ? (
              <p className={`text-center text-xs py-8 ${textMuted}`}>Nenhuma transação encontrada.</p>
            ) : (
              transacoesFiltradas.map(t => (
                <div key={t.id} className={`${cardClasse} rounded-2xl p-3.5 flex justify-between items-center text-xs md:text-sm`}>
                  <div className="flex items-center space-x-2.5">
                    ...
                    <div><span className="font-bold block">{t.descricao}</span><span className={`text-[10px] font-semibold text-emerald-500 block`}>{t.tipo === 'entrada' ? `Origem: ${t.origemEntrada}` : `Saiu de: ${t.poteNome}`}</span><span className={`text-[9px] ${textMuted}`}>{t.data}</span></div>
                  </div>
                  <div className="flex items-center space-x-2.5">
                    ...
                    <button onClick={() => removerTransacao(t.id)} className="text-slate-400 hover:text-rose-500 cursor-pointer"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                </div>
              ))
            )}
      ...
      {modalAporteSobra && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className={`${cardClasse} rounded-3xl p-5 max-w-sm w-full space-y-4 relative shadow-2xl`}>
            <button onClick={() => setModalAporteSobra(false)} className="absolute top-4 right-4 text-slate-400 cursor-pointer"><X className="w-5 h-5" /></button>
            ...
            <p className={`text-xs ${textMuted}`}>Transfira um valor que sobrou no seu Saldo Real direto para a formação de Patrimônio (Subtrai do saldo e injeta no pote).</p>
            
            <div className="space-y-3">
              <label className={`text-xs font-bold block ${textMuted}`}>Qual será o destino?</label>
              <select value={destinoAporteSobra} onChange={(e) => setDestinoAporteSobra(e.target.value as any)} className={`w-full ${inputBg} p-3 rounded-2xl text-xs font-bold border`}>
                <option value="nosso_patrimonio">🐷 Nosso Patrimônio (Automático)</option>
                <option value="patrimonio_manuela">👶 Poupança Manuela (Futuro Filha)</option>
              </select>

              <label className={`text-xs font-bold block ${textMuted}`}>Valor a transferir do Saldo (Máx: {formatarGrana(saldoUnicoReal)}):</label>
              <input type="number" placeholder="Valor R$" value={valorAporteSobra} onChange={(e) => setValorAporteSobra(e.target.value === '' ? '' : Number(e.target.value))} className={`w-full ${inputBg} p-3 rounded-2xl font-black text-lg focus:outline-none font-mono border`} />
            </div>
            
            <button onClick={efetivarAporteSobraPatrimonio} className="w-full bg-emerald-500 text-white font-black py-3 rounded-2xl shadow-lg text-sm cursor-pointer">Efetivar Aporte Extra</button>
          </div>
        </div>
      )}
      ...
      {modalAportePendente && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className={`${cardClasse} rounded-3xl p-5 max-w-sm w-full space-y-4 relative shadow-2xl`}>
            <button onClick={() => setModalAportePendente(false)} className="absolute top-4 right-4 text-slate-400 cursor-pointer"><X className="w-5 h-5" /></button>
            ...
            <input type="number" placeholder="Valor R$" value={aportePendenteValor} onChange={(e) => setAportePendenteValor(e.target.value === '' ? '' : Number(e.target.value))} className={`w-full ${inputBg} p-3 rounded-2xl font-black text-lg focus:outline-none font-mono border`} />
            <button onClick={async () => { await salvarDadosNaNuvem({ aportePendenteValor }); setModalAportePendente(false); }} className="w-full bg-emerald-500 text-white font-black py-3 rounded-2xl shadow-lg text-sm cursor-pointer">Salvar Alteração</button>
          </div>
        </div>
      )}
      ...
      {modalNovoPatrimonio && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className={`${cardClasse} rounded-3xl p-5 max-w-sm w-full space-y-4 relative shadow-2xl`}>
            <button onClick={() => setModalNovoPatrimonio(false)} className="absolute top-4 right-4 text-slate-400 cursor-pointer"><X className="w-5 h-5" /></button>
            <h3 className="text-base md:text-lg font-black">Adicionar Item ao Patrimônio</h3>
            <input type="text" placeholder="Nome (Ex: Imóvel, Veículo)" value={nomeNovoPatrimonio} onChange={(e) => setNomeNovoPatrimonio(e.target.value)} className={`w-full ${inputBg} p-3 rounded-2xl text-xs font-bold focus:outline-none border`} />
            <input type="number" placeholder="Valor R$" value={valorNovoPatrimonio} onChange={(e) => setValorNovoPatrimonio(e.target.value === '' ? '' : Number(e.target.value))} className={`w-full ${inputBg} p-3 rounded-2xl font-black text-lg focus:outline-none font-mono border`} />
            <button onClick={adicionarPatrimonioManual} className="w-full bg-emerald-500 text-white font-black py-3 rounded-2xl shadow-lg text-sm cursor-pointer">Salvar</button>
          </div>
        </div>
      )}
      ...
      {modalNovaMeta && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className={`${cardClasse} rounded-3xl p-5 max-w-sm w-full space-y-4 relative shadow-2xl max-h-[90vh] overflow-y-auto`}>
            <button onClick={() => setModalNovaMeta(false)} className="absolute top-4 right-4 text-slate-400 cursor-pointer"><X className="w-5 h-5" /></button>
            <h3 className="text-base md:text-lg font-black">Nova Meta Financeira</h3>
            <input type="text" placeholder="Nome da Meta (Ex: Viagem, Carro)" value={nomeNovaMeta} onChange={(e) => setNomeNovaMeta(e.target.value)} className={`w-full ${inputBg} p-3 rounded-2xl text-xs font-bold focus:outline-none border`} />
            <input type="number" placeholder="Valor Alvo R$" value={valorNovaMeta} onChange={(e) => setValorNovaMeta(e.target.value === '' ? '' : Number(e.target.value))} className={`w-full ${inputBg} p-3 rounded-2xl font-black text-base focus:outline-none font-mono border`} />
            
            <div className="space-y-1">
              <label className={`text-[11px] font-bold block ${textMuted}`}>Aportar Valor Inicial (Subtrai dos {formatarGrana(saldoUnicoReal)} disponíveis)</label>
              <input type="number" placeholder="Valor inicial para esta meta R$" value={guardadoNovaMeta} onChange={(e) => setGuardadoNovaMeta(e.target.value === '' ? '' : Number(e.target.value))} className={`w-full ${inputBg} p-3 rounded-2xl font-black text-base focus:outline-none font-mono border`} />
            </div>

            <input type="number" placeholder="Tempo de Conclusão (Meses)" value={mesesNovaMeta} onChange={(e) => setMesesNovaMeta(e.target.value === '' ? '' : Number(e.target.value))} className={`w-full ${inputBg} p-3 rounded-2xl font-black text-base focus:outline-none font-mono border`} />
            <button onClick={adicionarMeta} className="w-full bg-emerald-500 text-white font-black py-3 rounded-2xl shadow-lg text-sm cursor-pointer">Salvar Meta com Aporte</button>
          </div>
        </div>
      )}
      ...
      {modalDepositoMeta && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className={`${cardClasse} rounded-3xl p-5 max-w-sm w-full space-y-4 relative shadow-2xl`}>
            <button onClick={() => setModalDepositoMeta(false)} className="absolute top-4 right-4 text-slate-400 cursor-pointer"><X className="w-5 h-5" /></button>
            ...
            <div className="space-y-3">
              ...
              <select value={origemDepositoMeta} onChange={(e) => setOrigemDepositoMeta(e.target.value as any)} className={`w-full ${inputBg} p-3 rounded-2xl text-xs font-bold border`}>
              ...
              <input type="number" placeholder="Valor a Transferir R$" value={valorDepositoMeta} onChange={(e) => setValorDepositoMeta(e.target.value === '' ? '' : Number(e.target.value))} className={`w-full ${inputBg} p-3 rounded-2xl font-black text-lg focus:outline-none font-mono border`} />
            </div>
            <button onClick={efetivarTransferenciaMeta} className="w-full bg-emerald-500 text-white font-black py-3 rounded-2xl shadow-lg text-sm cursor-pointer">Efetivar Transferência</button>
          </div>
        </div>
      )}
      ...
      <nav className={`fixed bottom-0 inset-x-0 border-t p-1.5 flex justify-around items-center z-40 transition-colors duration-300 ${isDark ? 'bg-slate-950/95 border-slate-800' : 'bg-white/95 border-slate-200'}`}>
        <button onClick={() => navegarPara('dashboard')} className="flex flex-col items-center p-1.5 text-[10px] font-bold opacity-80 hover:opacity-100 cursor-pointer"><Home className="w-5 h-5 text-emerald-500" /> Início</button>
        <button onClick={() => navegarPara('extrato')} className="flex flex-col items-center p-1.5 text-[10px] font-bold opacity-80 hover:opacity-100 cursor-pointer"><List className="w-5 h-5 text-emerald-500" /> Extrato</button>
        <button onClick={() => setModalLancamento(true)} className="p-3 bg-emerald-500 text-white rounded-full shadow-lg shadow-emerald-500/30 -mt-5 active:scale-95 transition-transform cursor-pointer"><Plus className="w-5 h-5" /></button>
        <button onClick={() => navegarPara('metas')} className="flex flex-col items-center p-1.5 text-[10px] font-bold opacity-80 hover:opacity-100 cursor-pointer"><Target className="w-5 h-5 text-emerald-500" /> Metas</button>
        <button onClick={() => setMenuAberto(!menuAberto)} className="flex flex-col items-center p-1.5 text-[10px] font-bold opacity-80 hover:opacity-100 cursor-pointer"><MoreHorizontal className="w-5 h-5 text-emerald-500" /> Mais</button>
      </nav>

      {modalLancamento && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className={`${cardClasse} rounded-3xl p-5 md:p-6 max-w-sm w-full space-y-4 relative shadow-2xl`}>
            <button onClick={() => setModalLancamento(false)} className="absolute top-4 right-4 text-slate-400 cursor-pointer"><X className="w-5 h-5" /></button>
            <h3 className="text-base md:text-lg font-black">Novo Lançamento</h3>
            <div className={`flex ${inputBg} p-1 rounded-xl border`}>
              <button onClick={() => setTipoLancamento('saida')} className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all cursor-pointer ${tipoLancamento === 'saida' ? 'bg-rose-500 text-white' : textMuted}`}>Gasto (Saída)</button>
              <button onClick={() => setTipoLancamento('entrada')} className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all cursor-pointer ${tipoLancamento === 'entrada' ? 'bg-emerald-500 text-white' : textMuted}`}>Renda (Entrada)</button>
            </div>
            ...
            {tipoLancamento === 'entrada' ? (
              <div className="space-y-3">
                ...
                <div className={`flex ${inputBg} p-1 rounded-xl border`}>
                  <button onClick={() => setOrigemEntradaModal('Mercado Livre')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all cursor-pointer ${origemEntradaModal === 'Mercado Livre' ? 'bg-emerald-500 text-white' : textMuted}`}>Mercado Livre</button>
                  <button onClick={() => setOrigemEntradaModal('CLT')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all cursor-pointer ${origemEntradaModal === 'CLT' ? 'bg-emerald-500 text-white' : textMuted}`}>CLT</button>
                </div>
              </div>
            ) : (
              <div className="space-y-3">
                <label className={`text-xs font-bold block ${textMuted}`}>Categoria da Saída:</label>
                <select value={categoriaSaidaSelecionada} onChange={(e) => setCategoriaSaidaSelecionada(e.target.value)} className={`w-full ${inputBg} p-3 rounded-2xl text-xs md:text-sm focus:outline-none font-bold border`}>
            ...
            <button onClick={salvarLancamento} className="w-full bg-emerald-500 text-white font-black py-3 rounded-2xl shadow-lg shadow-emerald-500/20 text-sm cursor-pointer">Registrar Lançamento</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md z-50 flex justify-end" onClick={() => setMenuAberto(false)}>
          <div className={`${cardClasse} w-72 md:w-80 h-full p-5 space-y-5 overflow-y-auto relative border-l flex flex-col justify-between`} onClick={(e) => e.stopPropagation()}>
            <div className="space-y-5">
              <button onClick={() => setMenuAberto(false)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-200 cursor-pointer"><X className="w-5 h-5" /></button>
              <h3 className="text-lg font-black text-emerald-500">MEU IMPÉRIO</h3>
              <div className="space-y-1.5 text-xs md:text-sm font-bold">
                <button onClick={() => navegarPara('dashboard')} className={`w-full text-left p-3 rounded-2xl cursor-pointer ${isDark ? 'hover:bg-slate-800' : 'hover:bg-slate-100'} flex items-center gap-2.5`}><Home className="w-4 h-4 text-emerald-500" /> Início / Dashboard</button>
                <button onClick={() => navegarPara('onboarding')} className={`w-full text-left p-3 rounded-2xl cursor-pointer ${isDark ? 'hover:bg-slate-800' : 'hover:bg-slate-100'} flex items-center gap-2.5`}><Sliders className="w-4 h-4 text-emerald-500" /> Editar / Ajustar Plano</button>
                <button onClick={() => navegarPara('dividas')} className={`w-full text-left p-3 rounded-2xl cursor-pointer ${isDark ? 'hover:bg-slate-800' : 'hover:bg-slate-100'} flex items-center gap-2.5`}><CreditCard className="w-4 h-4 text-rose-500" /> Acompanhamento de Dívidas</button>
                <button onClick={() => navegarPara('metas')} className={`w-full text-left p-3 rounded-2xl cursor-pointer ${isDark ? 'hover:bg-slate-800' : 'hover:bg-slate-100'} flex items-center gap-2.5`}><Target className="w-4 h-4 text-emerald-500" /> Metas Financeiras</button>
                <button onClick={() => navegarPara('patrimonio')} className={`w-full text-left p-3 rounded-2xl cursor-pointer ${isDark ? 'hover:bg-slate-800' : 'hover:bg-slate-100'} flex items-center gap-2.5`}><Vault className="w-4 h-4 text-emerald-500" /> Patrimônio da Família</button>
                <button onClick={() => navegarPara('extrato')} className={`w-full text-left p-3 rounded-2xl cursor-pointer ${isDark ? 'hover:bg-slate-800' : 'hover:bg-slate-100'} flex items-center gap-2.5`}><List className="w-4 h-4 text-emerald-500" /> Extrato Completo</button>
              </div>
            </div>
            <div className="pt-4 border-t border-slate-800 space-y-2">
              <button onClick={reiniciarSistemaGeral} className="w-full bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 font-bold p-3 rounded-2xl flex items-center justify-center gap-2 text-xs transition-all cursor-pointer"><Trash2 className="w-4 h-4" /> Reiniciar Sistema (Zerar Tudo)</button>
              {onLogout && <button onClick={onLogout} className="w-full bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold p-3 rounded-2xl flex items-center justify-center gap-2 text-xs transition-all cursor-pointer"><LogOut className="w-4 h-4" /> Sair da Conta</button>}
            </div>
          </div>
        </div>
      )}
